...
1. **입력 항목**:
   ...
   - 이름 (실명 입력)
   - 기수 (숫자, 예: 24)
   - 주 활동 파트 (보컬(남), 보컬(여), 기타, 베이스, 드럼, 건반 등)
2. **제출 및 처리**:
...
### 3.2 로그인 (`/auth/login`)
1. 이메일과 비밀번호를 입력하고 `[로그인]` 클릭 (또는 Google 간편 로그인 지원).
2. **신규 방문자 유도**: 처음 방문하거나 계정이 없는 신규 부원 및 동문 회원을 위해 폼 하단에 강조된 **신규 회원가입 신청 안내 카드**(`소리로 크는 나무가 처음이신가요? [신규 회원가입 신청하기]`)를 제공하여 원클릭으로 `/auth/sign-up`으로 이동할 수 있도록 유도합니다.
3. 성공 시 서버 응답으로 HttpOnly 세션 쿠키가 브라우저에 저장되고 메인 또는 이전 페이지로 리다이렉트됩니다.
4. 실패 시 입력창 하단에 오류 문구(예: `"이메일 또는 비밀번호가 일치하지 않습니다."`)가 표시됩니다.
5. 이미 로그인한 상태에서 접근하면 로그인 폼을 보여주지 않고, 같은 사이트의 안전한 `redirect` 경로가 있으면 해당 경로로, 없으면 `/`로 이동합니다.
...
1. 구글 간편가입 후 필수 정보(기수, 세션 파트, 약관 동의)가 누락된 경우 자동 안내되는 폼이며, 보컬 세션은 `보컬(남)`과 `보컬(여)` 중 선택합니다.
2. 부원 정보 입력 중 페이지를 벗어날 때 `LeaveConfirmDialog` 경고 팝업이 표시됩니다.
3. 소셜 로그인만 완료하고 가입 신청서를 제출하지 않은 계정은 관리자 승인 대기 명단·알림 건수와 본인의 승인 대기 배지에 표시하지 않습니다. 기수(1 이상의 정수)와 공백이 아닌 세션이 저장된 신청만 표시합니다.
4. 미작성 계정이 `/profile`에 접근하면 이 등록 화면으로 안내합니다. 제출 성공 시 관리자 목록과 공통 헤더를 갱신합니다.
