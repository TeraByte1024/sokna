...
- 회원 정보 수정 폼:
  ...
  - 공연/행사 및 앱 푸시 알림 수신 동의 체크박스.
  - `[변경사항 저장하기]` 버튼.
- 앱 푸시 알림 카드:
  - `[이 기기 알림 켜기]`: 브라우저 권한 요청 및 FCM 토큰 등록.
  - `[이 기기 알림 끄기]`: 현재 브라우저 토큰 해제.
...
2. **정보 수정 및 저장 (`updateMyProfileAction`)**:
   ...
   - 푸시를 켜려면 먼저 수신 동의를 저장한 뒤 기기 알림 버튼을 눌러야 합니다.
   - 수신 동의를 끄면 해당 계정의 모든 기기 토큰이 삭제됩니다.
