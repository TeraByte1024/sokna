...
1. 사용자는 `/profile`의 기존 정보 수정 폼에서 `앱 푸시 알림 수신 동의` 여부를 확인하고 변경합니다. 별도의 푸시 설정 카드는 표시하지 않습니다.
   - 수신 동의 라벨 아래에 마지막 동의 시각을 한국 시간으로 표시합니다. 마이그레이션 이전 동의자는 시각이 null이므로 `동의함`만 표시합니다. 새 동의와 재동의는 DB 트리거가 시각을 기록하고, 철회 시 null로 초기화합니다.
2. 수신 동의 체크박스 바로 아래의 `이 기기에서 알림 받기` 토글 또는 헤더 프로필 팝업의 동일한 토글에서 현재 브라우저의 알림 권한과 토큰 등록을 관리합니다. 두 위치는 공통 기기 설정 훅과 토큰 변경 이벤트를 사용합니다.
3. 아직 수신에 동의하지 않은 사용자가 어느 토글이든 켜면 먼저 수신 동의 다이얼로그를 표시합니다. 확인 버튼의 직접적인 탭 동작에서 브라우저 알림 권한을 요청하고, 허용되면 수신 동의 저장과 기기 토큰 등록을 순서대로 수행합니다.
