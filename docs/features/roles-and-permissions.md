...
| **공연 (Gigs)** | 공연 목록 및 상세 조회 (`/gigs`, `/gigs/[id]`) | 전체 공개만 | 회원·전체 공개 | 회원·전체 공개 | 모든 범위 | 비공개는 관리자만, 회원 공개는 로그인한 모든 회원에게 표시 |
| | 공연 D-Day 및 총회 일정 확인 | O | O | O | O | 메인 카드 UI |
| | **신규 공연 등록 (`/gigs/new`)** | X | X | X | **O** | 관리자 전용 권한 |
| | **공연 세션 참여자(Performer) 배정** | X | X | X | **O** | 공연 생성 시 멤버 및 파트 지정 |
| | **공연 참가 신청 승인/무시 (`/admin/members`)** | X | X | X | **O** | 공연제목과 신청 정보를 한 줄 표로 표시, 미정 응답 제외 |
