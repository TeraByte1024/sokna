...
- **등록 자격 (보안 검사)**:
  ...
  - 마감 시각은 공통 유틸리티(`lib/nomination-deadline.ts`)로 계산하며, 기존 날짜 파싱 기준(`YYYY-MM-DD`는 UTC 자정)을 유지. 마감 시각과 같거나 이후이면 마감이며, 회의 날짜가 없거나 유효하지 않아도 일반 참여자의 등록을 차단.
  - 마감 후 추천 버튼은 링크가 없는 실제 `disabled` 버튼으로 표시하여 클릭 및 키보드 이동을 차단. 등록 URL 직접 접근 시 서버에서 선곡회의 목록으로 리다이렉트.
  - `addNomination` 및 호환 액션 `addSetlist`는 저장 직전에 공연의 최신 `meeting_date`와 서버 시각으로 마감을 재검사. 마감 전에 열어 둔 폼을 제출해도 `"선곡회의 접수가 마감되었습니다."` 오류를 반환하며 후보곡과 알림 큐를 생성하지 않음.
