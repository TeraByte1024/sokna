...
- 사용자가 `1024terrabyte@gmail.com`으로 등록한 후보곡 ID `102`, `104`에는 큐가 생성되지 않았던 사실을 확인했습니다.
- 큐 생성 client를 서버 secret client에서 후보곡 등록자의 인증 세션 client로 변경해 RLS 기반으로 이벤트가 보존되도록 수정했습니다.
- 최신 후보곡 ID `104`의 누락 큐를 ID `5`로 복구했으며 등록자는 `1024terrabyte@gmail.com`, 수신자는 `terabyte4096@gmail.com`으로 처리했습니다.
- 수신 계정의 Windows 토큰 2개와 Android 모바일 토큰 1개에 대해 FCM 성공 3건을 확인했습니다.
- 임시 검증 API와 로컬 서버는 검증 직후 제거·종료했습니다.
