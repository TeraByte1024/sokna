  ...
  if (error) {
    return (
      <p className="text-sm text-destructive">
        ...
        <code className="text-xs bg-muted px-1 rounded">{SUPABASE_GIGS_TABLE}</code>{" "}
    ...
    <div className="flex flex-col gap-8 w-full">
      ...
      {gigs.length === 0 ? (
        ...
        <ul className="flex flex-col gap-4">
          {gigs.map((gig) => (
            <li key={gig.id}>
              <Link href={`/gigs/${gig.id}/setlists`} className="block transition-transform hover:scale-[1.01] active:scale-100">
